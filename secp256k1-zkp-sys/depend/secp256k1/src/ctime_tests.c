...
static void run_tests(rustsecp256k1zkp_v0_10_0_context *ctx, unsigned char *key) {
...
#ifdef ENABLE_MODULE_FROST
    {
        ...
        rustsecp256k1zkp_v0_10_0_pubkey agg_vss_commitment[2];
        ...
        ret = rustsecp256k1zkp_v0_10_0_frost_share_agg(ctx, &agg_share, agg_vss_commitment, share_ptr, vss_ptr, pok_ptr, 2, 2, id_ptr[0]);
        SECP256K1_CHECKMEM_DEFINE(&ret, sizeof(ret));
        CHECK(ret == 1);
        SECP256K1_CHECKMEM_UNDEFINE(&agg_share, sizeof(&agg_share));
        CHECK(rustsecp256k1zkp_v0_10_0_frost_compute_pubshare(ctx, &pubshare[0], 2, id_ptr[0], agg_vss_commitment, 2));
        CHECK(rustsecp256k1zkp_v0_10_0_frost_compute_pubshare(ctx, &pubshare[1], 2, id_ptr[1], agg_vss_commitment, 2));
